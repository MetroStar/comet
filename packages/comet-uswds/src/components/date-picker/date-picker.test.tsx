...
describe('Date picker', () => {
  ...
  test('should set date to Jan 1 2020 using calendar', () => {
    ...
    // select year 2020
    fireEvent.click(
      baseElement.querySelector('.usa-date-picker__calendar__year-selection') as Element,
    );
    fireEvent.click(
      baseElement.querySelector('.usa-date-picker__calendar__year[data-value="2020"]') as Element,
    );

    // select month Jan
    fireEvent.click(
      baseElement.querySelector('.usa-date-picker__calendar__month-selection') as Element,
    );
    fireEvent.click(
      baseElement.querySelector('.usa-date-picker__calendar__month[data-value="0"]') as Element,
    );

    // select day 01
    fireEvent.click(
      baseElement.querySelector('.usa-date-picker__calendar__date[data-day="1"]') as Element,
    );

    expect(
      baseElement.querySelector<HTMLInputElement>(`input[name="${defaultName}"]`)?.value,
    ).toEqual('01/01/2020');
  });
  ...
  test('should set date to Jan 1 2020 using keyboard', async () => {
    ...
    // open calendar
    await fireEvent.click(baseElement.querySelector('.usa-date-picker__button') as Element);
  ...
  test('should be disabled when passing disabled', () => {
    ...
    expect(
      baseElement.querySelector('.usa-date-picker__external-input')?.hasAttribute('disabled'),
    ).toEqual(true);

    expect(baseElement.querySelector('.usa-date-picker__button')?.hasAttribute('disabled')).toEqual(
      true,
    );
  });
